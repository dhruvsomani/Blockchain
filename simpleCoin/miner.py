...
#     while random.randint(0, 9999) != 0:
#         new_blockchain = consensus(blockchain)
#         if new_blockchain:
#             print("SWITCHING TO ANOTHER CHAIN", new_blockchain[-1].index)
#             return False, new_blockchain
#     return 1, blockchain
...
def proof_of_work(last_proof, blockchain, a=None): 
    time_to_sleep = random.randint(5, 30)
    time.sleep(time_to_sleep)

    new_blockchain = consensus(blockchain, a)
    if new_blockchain:
        print("SWITCHING TO ANOTHER CHAIN", new_blockchain[-1].index)
        return False, new_blockchain
    return 1, blockchain
...
def mine(a, blockchain, node_pending_transactions):
    ...
    while True:
        ...
        #     last_block = create_block(last_block)
        ...
        if (type(last_block) == Block):
            ...
            print('SOMETHINGS WRONG. beta', last_block)
            last_proof = last_block['data']['proof-of-work']

        # Find the proof of work for the current block being mined
        # Note: The program will hang here until a new proof of work is found
        proof = proof_of_work(last_proof, BLOCKCHAIN, a)
        # If we didn't guess the proof, start mining again
        if not proof[0]:
            # Update blockchain and save it to file
            print('recvd blockchain of length', len(proof[1]))
            BLOCKCHAIN = proof[1]
            a.send(BLOCKCHAIN)
            continue
        else:
            ...
            NODE_PENDING_TRANSACTIONS = requests.get(url = config['MINER_NODE_URL'] + '/txion', params = {'update':config['MINER_ADDRESS']}).content
...
# {
#  "data": {"proof-of-work": 1, 
#           "transactions": [{"amount": 1, 
#                           "from": "network", 
#                           "to": "q3nf394hjg-random-miner-address-34nf3i4nfldhruv"
#                           }]
#           }, 
#  "hash": "8d2a392ea666316956c022b5fe3d7ba533deb887380cb409750fdc7fc8da9344", "index": 2, "timestamp": "1713122583.272058"}
def serialize_data(blockchain_str):
    ...
    for block in blockchain_str:
        ...
        # last_block.data = json.loads(last_block.data)
        last_block.data = ast.literal_eval(last_block.data)
...
def consensus(blockchain, a=None):
